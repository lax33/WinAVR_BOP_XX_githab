...
#define PROGRAMM_VERSION	0x050B	// RTU_V5 (NutOS v 5.1, WinAVR 2010) + комм для АС + задержка между переклюбчением и вычитыванием порта + 24bit номер + INI + add BOP + meas command on 23 port + off time + remove last simbol NUM in DISPLEY + 7 simbols of number + ПОДКЛЮЧ.К OTDR + ОШИБКА с номером
